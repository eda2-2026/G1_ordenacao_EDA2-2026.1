...
public class ControleEstoque {
    ...
    public ControleEstoque(ControleDados d) {
        for (int i = 0; i < d.getD().getEmpresa().getFiliais().size(); i++) {
            for (
                int j = 0;
                j <
                d.getD().getEmpresa().getFiliais().get(i).getCosmetico().size();
                j++
            ) {
                cosmeticos.add(
                    d
    ...
    /**
     * Retorna uma Array com todos os produtos da filial selecionada ordenados por um critério.
     *
     * @param d Dados.
     * @param pos Posição da filial.
     * @param criterio Critério de ordenação (Quantidade, Validade, Preco).
     * @return Retorna o estoque ordenado da filial.
     */
    public String[] getListaEstoqueOrdenada(
        ControleDados d,
        int pos,
        String criterio
    ) {
        ...
        todosProdutos.addAll(
            d.getD().getEmpresa().getFiliais().get(pos).getMedicamento()
        );
        todosProdutos.addAll(
            d.getD().getEmpresa().getFiliais().get(pos).getCosmetico()
        );

        switch (criterio) {
            case "Quantidade":
                todosProdutos.sort(
                    Comparator.comparingInt(Produto::getQuantidade).reversed()
                );
                break;
            case "Preco":
                todosProdutos.sort(
                ...
                break;
            case "Validade":
                todosProdutos.sort(Comparator.comparing(Produto::getValidade));
                break;
            default:
                // Sem ordenação específica, mantém a ordem de agregação (Meds depois Cosmeticos)
                break;
        }

        String[] estoqueArr = new String[todosProdutos.size()];
        for (int i = 0; i < todosProdutos.size(); i++) {
            estoqueArr[i] = todosProdutos.get(i).toString();
        }
        return estoqueArr;
    }
